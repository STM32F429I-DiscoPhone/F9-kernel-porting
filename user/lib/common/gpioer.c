...
void __USER_TEXT _gpio_config(struct gpio_cfg *cfg)
{
...
void __USER_TEXT _gpio_config_output(uint8_t port, uint8_t pin, uint8_t pupd, uint8_t speed)
{
	struct  gpio_cfg cfg = {
		.port = port,
		.pin = pin,
...
void __USER_TEXT _gpio_config_input(uint8_t port, uint8_t pin, uint8_t pupd)
{
	struct  gpio_cfg cfg = {
		.port = port,
		.pin = pin,
...
void __USER_TEXT _gpio_out_high(uint8_t port, uint8_t pin)
{
	*GPIO_ODR(port) |= (1 << pin);
}

void __USER_TEXT _gpio_out_low(uint8_t port, uint8_t pin)
{
	*GPIO_ODR(port) &= ~(1 << pin);
}

uint8_t __USER_TEXT _gpio_input_bit(uint8_t port, uint8_t pin)
{
